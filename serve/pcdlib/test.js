...
Lcd({ // physical pins
  ...
  lcd.drawString(0, 0, 'ok');
  await lcd.flushData();

  lcd.drawString(0, 20, 'okkkkkk');
  await lcd.flushData();
  await lcd.flushData(pi_logo);

  const qrdata = await QRCode.toString('asd');
  qrdata.split('\n').map((row, y) => _.forEach(row, (char, x) => {
    lcd.setpixel(x, 2*y, char === '▀' || char === '█');
    lcd.setpixel(x, 2*y+1, char === '▄' || char === '█');
  }));
  await lcd.flushData();
}).then(console.log).catch(x => console.error("THROWN", x));
